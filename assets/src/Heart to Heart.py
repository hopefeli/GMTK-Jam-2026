...
#
# Compilation command [pyinstaller --onefile --icon=ICON.ico "Heart to Heart.py"]
#

VERSION = "1.00"

header = ("Heart to Heart - VERSION : " + VERSION)
print(header)
...
def SetupDisplay ():
    ...
    windowIcon = pygame.image.load(".\\assets\\tiles\\MC-sprite-stand.png")
...
def Draw (screenScaled, records):
    ...
    for element in floaters:
        ...
        if element[0] + textLine.get_width() > nativeDisplaySize[0] - (tileSize * 0.5):
            element[0] = nativeDisplaySize[0] - textLine.get_width() - (tileSize * 0.5)
